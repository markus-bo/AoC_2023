...
public class PathfindingAStar<T> : IPathfindingAStar<T> where T : Node
{
    ...
    public IEnumerable<T> searchMinimumPath(T startNode, T endNode, Func<T, T, long> heuristic, Func<T, T, bool> pathFoundCondition, long initialLength = 0)
    {
        var pathEnd = searchMinimumPathLengthAStar(startNode, endNode, heuristic, pathFoundCondition, initialLength);
    ...
    {
        return searchMinimumPath(startNode, endNode, heuristic, pathFoundCondition, initialLength).Count() + initialLength;
    }

    private NodeAStar<T> searchMinimumPathLengthAStar(T startNode, T endNode, Func<T, T, long> heuristic, Func<T, T, bool> pathFoundCondition, long initialLength = 0)
    {
        ...
        var endNodeAStar = new NodeAStar<T>(endNode);
        ...
        while(openQueue.TryDequeue(out NodeAStar<T>? currentNode, out long Priority))
        {
            ...
            {
                if (pathFoundCondition(next.NodePosition,endNode) == true)
                {
